...
def get_3d_scores(model, save=False):
    ...
    layerwise_sem = np.zeros((len(layers), 2))
    for layer_i, layer in enumerate(tqdm(layers)):
        ...
        sem_1 = np.std([condition_1_index, set2_condition_1_index]) / np.sqrt(2)
        sem_2 = np.std([condition_2_index, set2_condition_2_index]) / np.sqrt(2)

        layerwise_sem[layer_i, 0] = sem_1
        layerwise_sem[layer_i, 1] = sem_2
    ...
    return layerwise_scores.T, layerwise_sem.T
